...
export const saveLesson = (lesson, meta) => {
  return new Promise((resolve, reject) => {
    ...
    const query = {
      "meta.lang": lang,
      "meta.lessonNumber": meta.lessonNumber,
    ...
    if (!meta.lessonNumber) {
      ...
      M.toast({
        html: `Sorry Lesson page number ${meta.lessonPageNumber} is already taken`,
      });
      $(`#${TOOL_CONFIG_MODAL_ID}-trigger`)[0].click();
      reject();
...
export const editLesson = ({ lessonId, lesson, meta }) => {
  ...
  if (_lesson && _lesson._id !== lessonId) {
    ...
    _lesson2 &&
    _lesson2._id == lessonId &&
    meta.lessonPageNumber !== 1
  ) {
    M.toast({
      html: `Sorry you can't change the lesson page number if it's '1'`,
    });
    $(`#${TOOL_CONFIG_MODAL_ID}-trigger`)[0].click();
    return;
  }
...
export const __editTool = ({ onEditTool, dispatch, tool }) => {
