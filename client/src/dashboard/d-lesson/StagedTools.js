...
function StagedTools() {
  ...
  return (
    <div className=" right col m2 staged-resource editor">
     ...
     <li className="collection-header "><h6>STAGGED</h6></li>
...
function RenderStaggedTools({tools,dispatch}){
  ...
  return tools.map((tool,key)=>(
    <li key={key} className="collection-item" onClick={()=>{dispatch(editTool(tool))}}>
    <div>{tool.label}
    <a href="#!" className="secondary-content">
    <i className="material-icons" >{tool.name}</i>
    <i className="material-icons red-text right" onClick={()=>alert('You will be able to remove tool at will')}> clear</i>
    </a></div></li>
  ))
