...
function Lessons({ lessons, match }) {
  ...
  return (
    <>
      <span to={"#"} className="btn right blue-grey lighten-2">
        {studenName}
      </span>
      <Link to={"/language_selector/?n=lessons"} className="btn right blue">
        Lessons
      </Link>
      <Link to={"/"} className="btn right cyan">
        Home
      </Link>
...
function RenderLessons({ dispatch, lessons, lang, match }) {
  ...
  return lessons.map((item, index) => (
    <li
      className={` ${
        item.meta.lessonNumber === parseInt(getUrlParam("n")) &&
        "blue-grey lighten-3"
      }`}
      key={index}
...
function Renderalphabet(props) {
  ...
  return ALPHABET.map((tool, index) => (
    <a className=" col m2" key={index} href="#!">
...
export default withTracker((props) => {
  ...
  return {
    lessons: COL_Lessons.find(query, {
      sort: { "meta.lessonNumber": 1 },
    }).fetch(),
  };
})(Lessons);
